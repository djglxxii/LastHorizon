...
- **Damage hierarchy:** typed-weapon energy absorbs enemy fire first → bare pea-shooter hits cost light Grid damage → enemy leaks past player cost heavy Grid damage → collision spends weapon energy first 1:1 against enemy HP, then any remaining enemy HP applies directly to Grid integrity (no cap); at 0 energy, no interception — enemy continues to leak normally.
...
- **Energy is dual-role:** the typed-weapon energy pool is consumed by BOTH firing the typed weapon AND absorbing enemy fire. It is shield and ammo at once. When it reaches 0, the typed weapon cannot fire until refilled, but the held family persists.
...
- **Ask opinion-based clarifying questions before activating a task.** When asked to create or move a task into `active/`, prompt the user for any questions, concerns, or details that the general plan does not cover. Focus on subjective/opinion-based gaps the design log and `PLAN.md` leave unspecified (feel, pacing, presentation, naming, judgment calls on scope edges) — not technical implementation concerns, which are your job to resolve. Surface these before you start work, not after.
