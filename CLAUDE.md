...
- **Weapon model:** permanent pea shooter + one temporary typed-weapon slot, levels 1–3, same-type levels & refills / different-type swaps to level 1. Per-weapon-family firing cost is allowed and expected (each family defines its own per-shot energy drain), creating a thrifty-vs-thirsty axis on top of pattern and damage.
